    ...
    public class reclamationservice implements IService<reclamation> {
        ...
        public void add(reclamation r, InputStream imageInputStream) {
            ...
            String query = "INSERT INTO Reclamation (id_reclamation, id_commande, date, type, description, statut, image) VALUES (?, ?, ?, ?, ?, ?, ?)";

            try {
                pst = conn.prepareStatement(query);
                pst.setInt(1, r.getId_reclamation());
                pst.setInt(2, latestIdCommande);
        ...
        public void update(reclamation reclamation) {
            String query = "UPDATE reclamation SET id_commande = ?, date = ?, type = ?, description = ?, statut = ?, image = ? WHERE id_reclamation = ?";
            try {
                pst = conn.prepareStatement(query);
